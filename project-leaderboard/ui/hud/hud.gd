...
@onready var controls_label: RichTextLabel = $controls/label
...
func show_controls(item: Holdable):	
	...
	controls_label.text = text % [item.display_name, "\n".join(controls_copy.values())]
...
func _on_event_added(details: EventData):
	...
	clone.name = Constants.get_lower_event_id(details.id)
...
func _on_event_solved(details: EventData):
	var lower_id = Constants.get_lower_event_id(details.id)
...
func _on_phase_changed(new_phase: Constants.Phase):
	...
	label.text = Constants.get_lower_phase(new_phase).to_upper()
