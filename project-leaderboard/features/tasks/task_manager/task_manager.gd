...
var current_source: Constants.TaskSources = Constants.TaskSources.NONE
...
func report_task_update(stat_name):
	...
	if current_task.mode == TaskData.Mode.STAT:
		var stat_enum: Constants.Stats = current_task.target as Constants.Stats
		
		if stat_enum == stat_name:
			add_progress = true
	elif current_task.mode == TaskData.Mode.ACTION:
		var stat_enum: Constants.Actions = current_task.target as Constants.Actions
	...
	if progress == target:
		...
		current_source = Constants.TaskSources.NONE
